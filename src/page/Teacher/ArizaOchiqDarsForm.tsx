...
import { FiBook, FiCalendar, FiClock, FiHome, FiUsers, FiHash, FiFileText, FiSend, FiArrowLeft } from "react-icons/fi";
...
function ArizaOchiqDarsForm() {
  ...
  const onSubmit = (data: FieldValues) => {
    ...
    apiClient.post("/adds", formData)
      .then(response => {
        console.log(response.data);
        toast.success("Application submitted successfully",);
        navigate("/teacher/ochiqdars");
        reset();
      })
      .catch(error => {
          ...
          <div className="card-body p-4" style={{ backgroundColor: '#f8fafc' }}>
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="row g-4 mb-4">
                ...
                {/* Vaqt */}
                <div className="col-md-6">
                  <label htmlFor="scheduleTime" className="form-label d-flex align-items-center" style={{
                    color: '#3a7bd5',
                    fontWeight: '500'
                  }}>
                  ...
                  <div className="input-group" style={{ borderRadius: '10px' }}>
                    ...
                    <input 
                      type="time" 
                      className={`form-control border-start-0 ${errors.scheduleTime ? 'is-invalid' : ''}`} 
                      style={{
                        borderColor: errors.scheduleTime ? '#dc3545' : '#e2e8f0',
                        borderRadius: '0 10px 10px 0',
                        transition: 'all 0.3s ease'
                      }}
                      id="scheduleTime" 
                  ...
                  {errors.scheduleTime && (
                    <div className="text-danger mt-1">{errors.scheduleTime.message}</div>
                  )}
                </div>
              </div>
              ...
              {/* Ariza matni */}
              <div className="mb-4">
                <label htmlFor="applicationText" className="form-label d-flex align-items-center" style={{
                  color: '#3a7bd5',
                  fontWeight: '500'
                }}>
                ...
                <div className="input-group" style={{ borderRadius: '10px' }}>
                  ...
                  <textarea 
                    rows={4} 
                    className={`form-control border-start-0 ${errors.applicationText ? 'is-invalid' : ''}`} 
                    style={{
                      borderColor: errors.applicationText ? '#dc3545' : '#e2e8f0',
                      borderRadius: '0 10px 10px 0',
                      transition: 'all 0.3s ease',
                      resize: 'none'
                    }}
                ...
                {errors.applicationText && (
                  <div className="text-danger mt-1">{errors.applicationText.message}</div>
                )}
              </div>
...
export default ArizaOchiqDarsForm;